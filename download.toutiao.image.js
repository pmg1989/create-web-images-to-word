  ...
  for (let curPageIndex = 0; curPageIndex < pageList.length; curPageIndex++) {
    ...
    const images = await page.evaluate(() =>
      Array.from(
        ...
        (el) => {
          ...
          return {
            src: $img.src,
            width: $img.width || $img.getAttribute("img_width"),
            height: $img.height || $img.getAttribute("img_height"),
          };
    ...
    for (let i = 0; i < images.length; i++) {
      const result = await download(
        images[i].src,
        `${IMAGES_DIR_PATH}/${pageTitle}/images`,
        `${i}x${images[i].width}x${images[i].height}.jpg`
      );
